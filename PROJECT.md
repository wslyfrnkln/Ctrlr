...
**Last Updated:** 2026-03-30 08:15
**Status:** PR merged, device QA passed, ios-designer skill upgraded with reference indexes | Health: green
**Type:** iOS App
**Version:** 1.0 (Build 1)
...
## Decisions
- 2026-03-30: Transport buttons (PLAY/RECORD/ARM/LOOP) made fully momentary — no persistent highlight state. UI cannot reflect DAW state without MIDI input, so any latch is misleading. Short-term fix; long-term fix requires MIDI input port + feedback CCs.
- 2026-03-30: `nonisolated(unsafe)` for `virtualSource`/`mapSource` in ConnectionManager — set once in init, never mutated. Needed to satisfy both `deinit` (nonisolated by definition) and `nonisolated route()`.
- 2026-03-30: Single `MacroDef` source of truth in AppModel.swift — `quickAccessMacros` (6) and `allMacros` (12) are canonical static arrays, replacing inline literals in ContentView.
- 2026-03-30: Dynamic `MIDIPacketList.Builder` sizing — hardcoded `byteSize: 256` caused EXC_BREAKPOINT on both iOS (MIDIManager) and Mac Helper (ConnectionManager). Now sizes from actual payload.
- 2026-03-30: SinAudio logo removed from ContentView bottom — shifts layout up, logo placement TBD.

## Next Session
<!-- Auto-generated by /closeout — 2026-03-30 07:30 -->

### Action Items
- [x] Open PR: `fix/code-review-2026-03` → `main` — merged as PR #2
- [x] Remove SinAudio logo from ContentView bottom
- [x] Phase 2 QA on physical iPhone — passed
- [x] XcodeBuildMCP tools activated
- [ ] Pick final app icon from `icon_concepts/` iterations — use `/ios-icon-designer` to continue
- [ ] Decide logo placement (removed from main view, needs new home or stays gone)
- [ ] MIDI input port in MIDIManager (`MIDIInputPortCreate`) + Ableton feedback CCs for transport state
...
| Date | Change |
|------|--------|
| 2026-03-30 | Research: SwiftUI interaction patterns for ios-designer — saved to vault: `research/ios-designer/2026-03-30-swiftui-interaction-patterns.md`. Reference + pattern indexes created in skill. |
| 2026-03-30 | Dynamic MIDIPacketList.Builder sizing (crash fix), logo removal, PR #2 merged, icon iteration started, `/ios-icon-designer` skill created |
| 2026-03-30 | Code review fixes: @MainActor, MIDIPacketList.Builder, momentary transport, macro dedup, dead code, font fix |
